{pkgs, ...}: let
  ...
  #enable-nouveau = pkgs.writeScriptBin "enable-nouveau" (builtins.readFile ./scripts/enable-nouveau.sh);
  #enable-nvidia = pkgs.writeScriptBin "enable-nvidia" (builtins.readFile ./scripts/enable-nvidia.sh);
in {
